 ...
 * @package Bedrock
 */
class BedrockYML extends BedrockNode {
	...
	public function get($setting) {		
		...
		foreach(explode('.', $setting) as $key) {
			...
			if(is_array($current[$key]))
				...
				return is_array($current[$key]) ? BedrockNode::create($key, $current[$key], $setting, $this) : self::parse_string($current[$key]);			
		}
		return is_array($current) ? BedrockNode::create($last_key, $current, $setting, $this) : self::parse_string($current);	
	}
